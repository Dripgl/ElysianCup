...
import { Lock, Mail, User, Trophy } from "lucide-react";
...
const RegisterScreen = () => {
  ...
  const [passwordStrength, setPasswordStrength] = useState(0); // Forza della password
  ...
  const evaluatePasswordStrength = (pwd: string) => {
    ...
    if (/[A-Z]/.test(pwd)) strength += 1; // Contiene maiuscole
    if (/[0-9]/.test(pwd)) strength += 1; // Contiene numeri
    if (/[^A-Za-z0-9]/.test(pwd)) strength += 1; // Contiene simboli
  ...
  const handlePasswordChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    evaluatePasswordStrength(newPassword); // Valuta la forza ad ogni digitazione
  };

  const getStrengthColor = () => {
    switch (passwordStrength) {
      case 0: return "bg-gray-400"; // Nessuna password
      case 1: return "bg-red-500"; // Molto debole
      case 2: return "bg-orange-400"; // Debole
      case 3: return "bg-yellow-400"; // Media
      case 4: return "bg-green-500"; // Forte
      case 5: return "bg-green-600"; // Molto forte
      default: return "bg-gray-400";
    }
  };

  const getStrengthWidth = () => {
    return `${(passwordStrength / 5) * 100}%`; // Calcola la larghezza in base alla forza massima di 5
  };
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    if (passwordStrength < 3) { // Puoi decidere la soglia minima di forza
        ...
        <Card className="bg-transparent border-none shadow-none text-white relative z-20">
          <CardHeader className="text-center pb-8">
            <div className="flex justify-center mb-8">
              ...
              >
                <Trophy className="w-12 h-12 text-white filter drop-shadow-[0_0_15px_rgba(255,255,255,0.9)]" />
              </motion.div>
            </div>
              ...
              <motion.div variants={inputVariants} className="space-y-2 relative">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    value={password}
                    onChange={handlePasswordChange} // Usa la nuova funzione
                    required
                    className="w-full pl-12 pr-4 py-3 bg-gray-800/70 border-gray-700 text-white placeholder-gray-500
                               focus:ring-green-500 focus:border-green-500 transition-all duration-200 text-base"
                  />
